...
export const login = async (values: z.infer<typeof LoginSchema>) => {
	...
	// }
	// // Verificar si el email del usuario ha sido confirmado
	// if (!existingUser.emailVerified) {
	// 	// Si el email no está verificado, generar un token de verificación
	// 	const verificationToken = await generateVerificationToken(existingUser.email);
	// 	// Enviar un correo electrónico de verificación al usuario
	// 	await sendVerificationEmail(verificationToken.email, verificationToken.token);
	// 	// Devolver un mensaje indicando que se ha enviado la confirmación de email
	// 	return { success: "Se ha enviado un correo de confirmación a tu dirección. Por favor, revisa tu bandeja de entrada para completar el proceso de verificación." };
	// }
	// try {
